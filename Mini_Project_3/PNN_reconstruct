...
if __name__ == "__main__":
    ...
    LL1 = np.load("givens LL1/LL1(279, 649, 850).npy")
    masked_LL1 = np.load("givens LL1/masked_LL1(279, 649, 850).npy")  # shape: (279, 649, 850)
    allPoses = np.load('givens LL1/poses_downsampled(3, 4, 279).npy')

    # for voxel_size in [0.0001, 0.00023813, 0.00025, 0.0005, 0.00075,  0.001, 0.0025, 0.005, 0.01, 0.025, 0.05, 0.1]:
    #     voxel_grid, origin, elapsed_time, point_cloud = PNN_reconstruct(masked_LL1, allPoses, voxel_size, multiple_inputs='average')

    #     np.save(f"{voxel_size}_avg_uncropped_masked_LL1.npy", voxel_grid)
    ...
    voxel_grid, origin, elapsed_time, point_cloud = PNN_reconstruct(masked_LL1, allPoses, 0.001, multiple_inputs='average')
